...
test.describe('Dashboard', () => {
  test('carrega com sidebar, mapa e ranking', async ({ page }) => {
    await page.goto('/')
    // Sidebar
    await expect(page.getByRole('heading', { name: 'Pesca PR' }).first()).toBeVisible({ timeout: 10000 })
    ...
    // Mapa carrega (forecast pode demorar)
    await expect(page.locator('.leaflet-container')).toBeVisible({ timeout: 60000 })

    // Ranking
    await expect(page.getByRole('heading', { name: /^Ranking — / })).toBeVisible({ timeout: 60000 })
  })

  test('botões de dia são clicáveis e mudam o conteúdo', async ({ page }) => {
    await page.goto('/')
    // Forecast API pode demorar ~25s na primeira chamada
    await expect(page.locator('button:has-text("Hoje")')).toBeVisible({ timeout: 60000 })
  ...
  test('espécies e iscas em alta aparecem', async ({ page }) => {
    await page.goto('/')
    await expect(page.locator('text=Espécies em alta')).toBeVisible({ timeout: 60000 })
    await expect(page.locator('text=Iscas em alta')).toBeVisible()
  })

  test('navios fundeados aparecem no ranking', async ({ page }) => {
    await page.goto('/')
    await expect(page.locator('text=Navios fundeados')).toBeVisible({ timeout: 60000 })
  })
